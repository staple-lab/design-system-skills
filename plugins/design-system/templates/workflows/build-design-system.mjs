    ...
    agent(
      brief(
        ...
        [
          ...
          'opens/closes/expands, the behavioural test suite, the <Name>.meta.json registry file,',
          'and the <Name>.examples.tsx live-preview file. The bar is not "it works" — it is that a',
          'reviewer cannot tell it was written by someone different from the rest of the library.',
          '',
          'The examples file is NOT optional and is not the meta file\'s code strings. The inventory',
          'site globs components/*/*.examples.tsx and renders the real component; without it every',
          'example on your page is a dead code block reading "Code only — add <Name>.examples.tsx".',
          'Export one NO-PROPS component per meta example, named as that example\'s title with every',
          'non-alphanumeric character stripped ("Icon slots" -> Iconslots). Anything that opens must',
          'render a TRIGGER with local state — never an always-open overlay, it portals to the body',
          'and covers the whole site. Layout primitives need children with a visible surface, or the',
          'demo shows nothing. Both themes, semantic tokens only.',
          '',
