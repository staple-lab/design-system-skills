 ...
 *
 * Emits .design-system/registry.json and (optionally) AGENTS.md + llms.txt.
 *
 ...
 *   --check   fail if the committed registry (or llms.txt) is stale (run this in CI)
 *   --agents  also write AGENTS.md and llms.txt
 *
...
async function build() {
  ...
  const aiSurfaces = args.has('--agents') || config.ai?.generateAgentsMd;
  ...
  if (args.has('--check')) {
    ...
    if (aiSurfaces) {
      ...
      const llmsCurrent = existsSync(llmsPath) ? readFileSync(llmsPath, 'utf8') : '';
      if (llmsCurrent !== renderLlmsTxt(registry)) {
        console.error(`✗ llms.txt is stale. Run \`npm run registry\` and commit the result.`);
        process.exit(1);
      }
    }
  ...
  if (aiSurfaces) {
    writeAgentsMd(registry);
    writeFileSync(join(ROOT, 'llms.txt'), renderLlmsTxt(registry));
...
// ---------------------------------------------------------------------------
// llms.txt — the llmstxt.org index: where an LLM should look, in one page
// ---------------------------------------------------------------------------

/**
 * AGENTS.md carries the rules; llms.txt is the MAP — a short index of the
 * machine-readable artifacts, one line per component, and the commands that
 * regenerate everything. Pure function of the registry so --check can diff it.
 */
function renderLlmsTxt(registry) {
  const { system, counts } = registry;
  const statusLine = Object.entries(counts.byStatus).map(([s, n]) => `${n} ${s}`).join(', ');

  const componentLines = registry.components
    .map((c) => `- [${c.name}](${c.sourcePath.split(sep).join('/')}): ${c.status} — ${c.summary || c.description || 'no summary yet'}`)
    .join('\n');

  return `# ${system.name}

> ${config.displayName ?? system.name} — a ${system.primitives}/${system.cssSystem} React design system with ${counts.total} component${counts.total === 1 ? '' : 's'} (${statusLine}). Everything below is generated from one registry; read the linked files rather than guessing component or token names.

Generated by scripts/build-registry.mjs — do not edit by hand.

## Machine-readable sources

- [Component registry](${PATHS.registry.split(sep).join('/')}): every component — props, keyboard map, a11y contract, tokens consumed, examples, adoption
- [Design tokens](${PATHS.tokens}/dist/tokens.json): flat resolved token map — value per theme, tier, description
- [Agent rules](AGENTS.md): the house rules for writing code against this system
...
- \`npm run tokens\`: rebuild ${PATHS.tokens}/dist from the DTCG sources (runs the contrast gate)
- \`npm run registry\`: regenerate the registry, AGENTS.md and this file
- \`npm run lint\`: eslint + stylelint, including token enforcement
- \`npm run test\`: vitest — writes the report that feeds test status into the registry
`;
}
